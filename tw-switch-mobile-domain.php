...
class tw_switch_mobile_domain
{
	...
	public function __construct(){
		...
		if($this->mobile_theme != '' && ($this->current_domain == $this->mobile_domain || wp_is_mobile() || $this->is_bot()))
    ...
    public function tw_switch_theme(){
    	...
    	add_filter('stylesheet', array($this, 'tw_add_filter_switch_stylesheet'), 0);
    	add_filter('template',   array($this, 'tw_add_filter_switch_template'), 0);
    ...
    public function tw_add_filter_switch_stylesheet(){
    ...
    public function tw_add_filter_switch_template(){
    ...
    public function tw_show_admin_page(){
    	...
    	<div class="wrap">
    		...
    		<form method="post" action="options.php">
    			...
    			<?php
    			$theme_names   = wp_get_themes();
    			$current_theme = get_current_theme();
    			?>
    			<select name="tw_mobile_theme">
    				<?php foreach($theme_names as $theme_name):?>
    				<?php if(($this->mobile_theme == $theme_name['Name']) || (($this->mobile_theme == '') && ($theme_name['Name'] == $current_theme))):?>
    				<option value="<?php echo $theme_name['Name']?>" selected="selected"><?php echo $theme_name['Name']?></option>
    			    <?php else:?>
    			    <option value="<?php echo $theme_name['Name']?>"><?php echo $theme_name['Name']?></option>
    			    <?php endif;?>
    			    <?php endforeach;?>
    			</select><br/>
                ...
                <select name="tw_bot_theme">
                    <?php foreach($theme_names as $theme_name):?>
                    <?php if(($this->bot_theme == $theme_name['Name']) || (($this->bot_theme == '') && ($theme_name['Name'] == $current_theme))):?>
                    <option value="<?php echo $theme_name['Name']?>" selected="selected"><?php echo $theme_name['Name']?></option>
                    <?php else:?>
                    <option value="<?php echo $theme_name['Name']?>"><?php echo $theme_name['Name']?></option>
                    <?php endif;?>
